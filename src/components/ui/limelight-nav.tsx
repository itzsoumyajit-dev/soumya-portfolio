...
export const LimelightNav = ({
  ...
  // Expose active index update for external sync (e.g. scroll spy)
  useLayoutEffect(() => {
  ...
  return (
    <nav className={`relative inline-flex items-center h-16 rounded-2xl bg-background/80 backdrop-blur-md text-text border border-border px-2 ${className}`}>
      ...
      <div 
        ref={limelightRef}
        className={`absolute top-0 z-10 w-11 h-[5px] rounded-full bg-accent shadow-[0_30px_15px_var(--accent)] ${
          isReady ? 'transition-[left] duration-400 ease-in-out' : ''
        } ${limelightClassName}`}
        style={{ left: '-999px' }}
      >
        <div className="absolute left-[-30%] top-[5px] w-[160%] h-14 [clip-path:polygon(5%_100%,25%_0,75%_0,95%_100%)] bg-gradient-to-b from-accent/30 to-transparent pointer-events-none" />
      </div>
    </nav>
  );
};
